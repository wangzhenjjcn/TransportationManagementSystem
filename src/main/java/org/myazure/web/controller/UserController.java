...
public class UserController  extends BaseController{
	...
	public void login(HttpServletRequest request, HttpServletResponse response)
		...
		if (!webUserService.login(username, password, JSSESSION,logonIp)) {
			...
			response.addCookie(new Cookie("4682543566466", System.currentTimeMillis()+""));
		}else {
			  loginStatusResponse = new StatusResponse(
					"user:"+username+" logon sucess", 0, true);
		}
	...
	@RequestMapping(path = "/getDrivers", method = { RequestMethod.POST,
			RequestMethod.GET })
	public void getDrivers(HttpServletRequest request, HttpServletResponse response)
			throws IOException {
